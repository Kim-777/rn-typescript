...
const PersonEnterExit: FC<PersonProps> = ({person, deletePressed}) => {
  ...
  const iconAnimations = useMemo(() => iconAnimValues.map((animValue) => Animated.spring(animValue, {useNativeDriver: true, toValue: !started ? 1 : 0})), [started])
  const enterAnimation = useCallback(() => {
    Animated.sequence([
      Animated.timing(leftToRightAnimValue, {
        useNativeDriver: true, toValue: 1, duration: 1 * 1000, easing: Easing.bounce
      }),
      Animated.spring(opacityAnimValue, {useNativeDriver: true, toValue: 1}),
      Animated.timing(topBottomAnimValue, {
        useNativeDriver: true, toValue: 1, duration: 1 * 1000, easing: Easing.circle
      }),
      ...iconAnimations
    ]).start(toggleStarted)
  }, [])
  ...
  const exitAnimation = useCallback(() => {
    Animated.sequence([
      ...
      Animated.timing(leftToRightAnimValue, {
        useNativeDriver: true, toValue: 0, duration: 0.3 * 1000, easing: Easing.bounce
      })
    ]).start(deletePressed)
  }, [started])
  ...
  const centerIconStyle = useTransformStyle({
    translateX: interpolate(iconAnimValues[1], !started ? [1200, 0] : [0, 1200])
  })
    ...
    <Animated.View style={[styles.view, enterLeaveTransformStyle]}>
      ...
      <View style={[styles.rightView]}>
        ...
        <View style={[styles.dateView]}>
          ...
          <Icon name="trash-can-outline" size={26} color={Colors.lightBlue500} onPress={exitAnimation}/>
        </View>
        ...
        <View style={[styles.countsView]}>
          <AnimatedIcon style={[leftIconStyle]} name="comment" size={24} color={Colors.blue500}/>
          <AnimatedIcon style={[centerIconStyle]} name="twitter-retweet" size={24} color={Colors.purple500}/>
          <AnimatedIcon style={[rightIconStyle]} name="heart" size={24} color={Colors.red500}/>
        </View>
      </View>
    </Animated.View>
  )
