...
const PersonParallel: FC<PersonProps> = ({person, deletePressed}) => {
  ...
  const animations = useMemo(() => animValues.map((animValue) => Animated.spring(animValue, {useNativeDriver: true, toValue: !started ?  1: 0})), [started])
  const avatarPressed = useCallback(() => {
    Animated.parallel(animations).start(toggleStarted)
  }, [started])

  const leftIconStyle = useTransformStyle({
    translateX: interpolate(animValues[0], [-1200, 0])
  })
    ...
    <View style={[styles.view]}>
      ...
      <View style={[styles.rightView]}>
        ...
        <View style={[styles.countsView]}>
          ...
          <AnimatedIcon style={[rightIconStyle]} name="comment" size={24} color={Colors.red500}/>
        </View>
      </View>
    </View>
  );
